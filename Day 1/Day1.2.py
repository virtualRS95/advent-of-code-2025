initial_number = 50
input_file = "input.txt"
# input_file = "debuginput3.txt"

# open the input file
with open(input_file) as ifp:
    instructions = ifp.readlines()

# replace L and R in the instructions with - and +
for i in range(len(instructions)):
    if instructions[i][0] == "L":
        instructions[i] = -1* int(instructions[i][1:])
    else: instructions[i] = int(instructions[i][1:])

"""
Task 1 Iteratively add instructions to initial number and count intermediate zeros
Task 2: Also count every time 0 is passed during an instruction
    50+60 --> 110            Add 1
    50-50 --> 0              Add 1
    However: 50+150 --> 200  Add only 2 (1 for rotation past 0 and 1 for landing on it)
    -> If the turn ends on a zero, subtract one from the hundreds zero increment
    Warning: Going negative after landing on 0 should not increment!
"""
zeros = 0
number = initial_number
for instruction in instructions:
    # Algorithm counts one too many zeros when going from a 0 value to negatives
    if instruction < 0 and number == 0 and instruction % 100 != 0:
        zeros -= 1

    old_number = number

    # modulo operator because number range is 0-99., 0 minus 5 --> 95
    raw_number = number+instruction
    number = raw_number % 100
    
    # use floor division to determine how many times we passed 0
    hundreds_raw_number = raw_number//100
    hundreds_number = number//100
    times_passed_zero = abs(hundreds_raw_number-hundreds_number)
    zeros += times_passed_zero

    # algorithm counts one too few zeros when going from 1>number>99 to 0
    if number == 0 and times_passed_zero == 0:
       zeros += 1

    # algorithm counts one too few when going from a positive value to negative 100s
    if instruction < 0 and number == 0 and raw_number < -99 and old_number > 0:
        zeros += 1

    print(
        f"debug:\n"
        f"instruction={instruction}\n"
        f"Raw_number={raw_number}\n"
        f"number={number}\n"
        f"hundreds_raw_number={hundreds_raw_number}\n"
        f"hundreds_number={hundreds_number}\n"
        f"times_passed_zero={times_passed_zero}\n"
        f"on zero added={int(number == 0 and times_passed_zero == 0)}\n"
        f"zeros={zeros}\n"
        )
    


print(f"The number of times zero was passed or hit is {zeros}")